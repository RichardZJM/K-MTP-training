...
\begin{abstract}
Machine learning models have been of recent interest in the modelling of interatomic potential for molecular dynamics simulations. One such model is the MTP, a linear regression-based model which employs an active learning framework to train potentials. In exploring the MTPs uses for the Queen's Nuclear Materials group, we utilize a bottom-up training scheme approach to train potassium metals MTPs which we then validate. We find that the scheme produces effective MTP with less cost than more direct approaches. We also note that the MTP has generally strong accuracy that increases with the MTP's level of representation, yielding strong performance in both artificial validation datasets and comparisons against \textit{ab inito} molecular dynamics run for MTP levels above 6. This translates into excellent liquid radial distribution function predictions although more specific training regimes are likely needed for properties that involve the precise prediction of a small number of configurations. Overall for a solid-liquid potassium model, we would recommend an MTP level of no less than 10.
\end{abstract}
