...
Thus, for each training configuration, we can formulate a vector of the per-configuration value of the basis functions, $b(\textrm{cfg})$. Taking these vectors of rows, we can form a matrix with a height equal to the number of training configurations and a width equal to the number of basis functions in the chosen level of MTP. 
